...
class F16PxResolver(ResolveUrl):
    name = 'F16Px'
    domains = ['f16px.com', 'bysevepoin.com', 'bysebuho.com', 'g9r6.com', 'bysesayeveum.com', 'bysetayico.com']
    pattern = r'(?://|\.)((?:f16px|byse(?:vepoin|buho)|g9r6)\.com)/(?:e|d|x3uk)/([0-9a-zA-Z]+)'

    def get_media_url(self, host, media_id):
        web_url = self.get_url(host, media_id)
        headers = {
            'User-Agent': common.FF_USER_AGENT,
            'Referer': 'https://{host}/'.format(host=host)
        }
        ...
        if sources:
            ...
            if uri.startswith('/'):
                uri = 'https://{host}{uri}'.format(host=host, uri=uri)
